<template>
  <div>
    <h2 class="title-camera">Selecione uma câmera para iniciar o treinamento</h2>
    <ul class="camera-menu" v-if="menuCams.length > 0">
      <li class="camera-menu__item" v-on:click="changeCamera(camera.camId)"  :key="camera.camId" v-for="camera in menuCams">
        <router-link :to="{ name: 'camera-training', params: {id: camera.camId}, query: {canvasMode: 'show'}}">
